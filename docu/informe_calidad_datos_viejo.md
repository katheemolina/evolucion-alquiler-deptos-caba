...
### Variables del dataset maestro integrado (`dataset_maestro.csv`)

| Variable | Significado | Tipo de dato | Clasificación |
|----------|-------------|--------------|---------------|
| `comuna` | Número de comuna de la CABA (1 a 15) | Entero | Identificador / Categórica |
| `periodo_trim` | Trimestre calendario (formato `YYYY-QN`) | Texto (`2018-Q1`) | Ordinal (tiempo) |
| `ambientes` | Cantidad de ambientes del departamento (1, 2 o 3) | Entero | Categórica ordinal |
| `precio_promedio_pesos` | Precio promedio de publicación en pesos argentinos | Float | Numérica continua |
| `provisorio` | Indica si el dato de precio fue marcado como provisorio (*) en la fuente | Booleano | Categórica |
| `sueldo_basico_admin_a` | Sueldo básico trimestral Administrativo A (Empleados de Comercio, SEC La Plata) | Float | Numérica continua |
| `n_meses_sueldo` | Cantidad de meses promediados en el trimestre salarial | Entero | Numérica |
| `ratio_alquiler_sueldo_pct` | `(precio / sueldo) × 100` — % del sueldo básico que representa el alquiler | Float | Numérica continua (derivada) |
| `meses_sueldo_para_alquiler` | `precio / sueldo` — meses de sueldo básico equivalentes al alquiler | Float | Numérica continua (derivada) |
| `superficie_total_m2` | Suma de m² publicados en alquiler en la comuna (agregado desde barrios) | Float | Numérica continua |
| `cantidad_deptos_estimada` | Estimación de unidades ofrecidas = superficie total / superficie promedio ciudad | Float | Numérica continua (derivada) |

### Variables del dataset barrios mensual (`dataset_barrios_mensual.csv`)

| Variable | Significado | Tipo de dato | Clasificación |
|----------|-------------|--------------|---------------|
| `barrio` | Nombre del barrio de la CABA | Texto | Identificador / Categórica |
| `periodo` | Mes calendario (formato `YYYY-MM`) | Texto | Ordinal (tiempo) |
| `ambientes` | Cantidad de ambientes (1, 2 o 3) | Entero | Categórica ordinal |
| `superficie_total_m2` | Metros cuadrados publicados en alquiler en el barrio y mes | Float | Numérica continua |
| `superficie_promedio_m2` | Superficie promedio ciudad usada para el cálculo | Float | Numérica continua |
| `cantidad_deptos_estimada` | Unidades estimadas en el barrio | Float | Numérica continua (derivada) |
| `sueldo_basico_admin_a` | Sueldo básico mensual Administrativo A (contexto sector comercio) | Float | Numérica continua |
| `sueldo_interpolado` | `True` si el sueldo fue estimado por interpolación lineal | Booleano | Categórica |
...
3. **¿Qué proporción del sueldo promedio de un empleado de comercio debería destinarse al alquiler de un monoambiente (1 ambiente) según comuna y en el tiempo, y qué comunas resultan menos asequibles frente a un umbral de referencia (p. ej. 30 % del ingreso)?**  
   *Variables:* `precio_promedio_pesos` (con `ambientes = 1`), sueldo promedio empleados de comercio (dataset complementario), `comuna`, `periodo_trim`.  
   *Variable derivada:* `ratio_alquiler_sueldo_pct = (precio_promedio_pesos / sueldo_promedio_comercio) × 100`.  
   *Viz sugerida:* líneas del ratio por comuna; mapa coroplético de asequibilidad; línea de referencia al 30 %; facetas por trimestre o slider temporal en el dashboard.

   > **Nota metodológica:** el sueldo de comercio es un *proxy* de ingreso (no representa a todos los inquilinos). Conviene documentar la fuente, alinear la periodicidad (trimestral o promedio mensual del trimestre) y acotar el análisis al mismo rango temporal (`≥ 2018-Q1`). La pregunta sobre completitud de precios (`///`, datos provisorios) queda cubierta en las secciones 3.2 y 3.4 del informe y puede mostrarse como indicador de calidad en el dashboard, sin ser una pregunta de análisis principal.
...
| `dataset_maestro.csv` | Dataset integrado comuna × trimestre × ambientes (desde 2018-Q1) |
| `dataset_barrios_mensual.csv` | Superficie y deptos estimados por barrio × mes |
